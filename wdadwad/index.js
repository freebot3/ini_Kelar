...
const { autoTyping, autoRecord, sendReadReceipt } = require('./Auto_ketik_rekam_ceklis_biru.js'); // Impor fungsi dari Auto_typing_record_sendReadReceipt.js
const { sendWelcomeGoodbyeMessage, handleGroupInfoChange, handleAdminStatusChange } = require('./fitur'); // Impor fungsi dari fitur.js
const { execSync } = require('child_process'); // Impor child_process untuk menjalankan perintah npm
const { sendNotification } = require('./notifpesantersambung'); // Impor fungsi dari notifpesantersambung.js
...
const { resetAllWarningCounts, antilinkgc, antilinkchannel, handleParticipantUpdate } = require('./antigroupchannel'); // Impor fungsi dari antigroupchannel.js
const { antivideo } = require('./Antivideo'); // Impor fungsi antivideo
const { handleStatusUpdate, saveCounts, loadCounts } = require('./emoji');
const { antispam, isSpam } = require('./antispam'); // Impor fungsi antispam dan isSpam
    ...
    client.ev.on("messages.upsert", async (chatUpdate) => {
      try {
        ...
        if (m.key && !m.key.fromMe && m.message.protocolMessage?.type !== 2) { // 2 is the protocol message type for status delete
          if (config.antispam && isSpam(m.key.remoteJid)) {
            console.log(`Pesan dari ${m.key.remoteJid} terdeteksi sebagai spam.`);
            return;
          }
          ...
          await handleStatusUpdate(client, m); // Panggil fungsi handleStatusUpdate
          ...
          await antispam(client, m); // Panggil fungsi antispam
          viewCount++;
